...
namespace Compilador
{
    ...
    public partial class Form1 : Form
    {
        ...
        // [Menu] - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        private void nuevoArch(object sender, EventArgs e)
        {//[Nuevo archivo]
            ruta = "";//sin guardar
        ...
        {//compilar
            ...
            Process lexico = new System.Diagnostics.Process();
            lexico.StartInfo.FileName = "AL.exe";
            lexico.StartInfo.Arguments = "\"" + ruta + "\"";
            lexico.EnableRaisingEvents = true;
            lexico.Exited += new System.EventHandler(Lexico_Exited);
            lexico.Start();
        }
        ...
        private void CambiarTexto(RichTextBox richBox, string texto)
        {
            if (richBox.InvokeRequired)
            {
                CambiarTextoCallback d = new CambiarTextoCallback(CambiarTexto);
                this.Invoke(d, new object[] { richBox, texto });
            }
            else
            {
                richBox.Text = texto;
